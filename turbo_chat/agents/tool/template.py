...
Tools:
{% for tool in tools %}
{% set tool_signature = tool | fn_signature -%}
- {{tool.__name__}}: {{tool | fn_doc or "Use tool"}}. This tool{% if tool_signature %} requires a valid json as input that includes these keys: ({{tool_signature}}){% else %} does not require an input{% endif %}.
{% endfor %}
